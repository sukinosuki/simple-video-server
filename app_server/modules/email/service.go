...
func (s *_service) Send(c *core.Context, data *SendEmail) (string, error) {
...
// 重置密码
func (s *_service) handleResetPassword(c *core.Context, data *SendEmail) (string, error) {
	// 获取到邮箱
	//uid := *c.UID
...
// 处理注册
func (s *_service) handleRegister(c *core.Context, data *SendEmail) (string, error) {
	//	TODO: 校验邮箱是否已注册
	//var count int64
...
func _sendEmail(targetEmails []string, text, subject string) error {
	...
	err := newEmail.Send("smtp.qq.com:25", smtp.PlainAuth("", config.Email.Email, config.Email.AuthCode, "smtp.qq.com"))
