...
func (dao *_dao) UpdateProfile(user *models.User) error {
	tx := dao.db.Model(dao.model)

	// 更新指定字段
	err := tx.Where("id = ?", user.ID).
		Select("avatar", "nickname", "gender", "birthday").Updates(user).Error

	return err
}

// FindByEmailAndPassword find by email and password
