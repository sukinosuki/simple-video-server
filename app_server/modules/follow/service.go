...
func (s *_service) FollowScores(c *core.Context, query *UserFollowRankQuery) (users []*UserFollowerRankResSimple, err error) {
	...
	//每个user赋值对应的score
	arr.ForEach(users, func(item *UserFollowerRankResSimple, index int) {
...
func (s *_service) GetUserFollower(c *core.Context, userId uint, query *UserFollowerQuery) ([]*UserFollowerResSimple, error) {
	...
	var users []*UserFollowerResSimple
	err = s.db.Model(&models.User{}).Where("id in ?", ids).Find(&users).Error
